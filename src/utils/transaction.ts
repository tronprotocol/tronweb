...
const buildUnDelegateResourceContract = (value, options) => {
  ...
  unDelegateResourceContract.setReceiverAddress(
...
const buildCreateSmartContract = (value, options) => {
  ...
  if (bytecode) {
    const bytecodeParameter =
      bytecode.replace(/^0x/, "") + parameter.replace(/^0x/, "");
...
const buildAssetIssueContract = (value, options) => {
  ...
  if (frozen_supply) {
    ...
    frozenSupplyContract.setFrozenAmount(
      frozen_supply.length
        ? frozen_supply[0].frozen_amount
        : frozen_supply.frozen_amount
    );
    frozenSupplyContract.setFrozenDays(
      frozen_supply.length
        ? frozen_supply[0].frozen_days
        : frozen_supply.frozen_days
    );
...
const buildAccountPermissionUpdateContract = (value, options) => {
  ...
  const buildPermission = (data) => {
    ...
    const { type, id, permission_name, threshold, parentId, operations, keys } =
      data;
...
const buildUpdateAssetContract = (value, options) => {
  ...
  const { owner_address, description, url, new_limit, new_public_limit } =
    value;
  updateAssetContract.setOwnerAddress(fromHexString(owner_address));
  if (description) {
    updateAssetContract.setDescription(
      fromHexString(description.replace(/^0x/, ""))
    );
  }
  if (url) {
