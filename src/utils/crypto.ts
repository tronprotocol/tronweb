...
export function getRowBytesFromTransactionBase64(
  base64Data: string
): Uint8Array {
...
export function getPubKeyFromPriKey(priKeyBytes: BytesLike) {
  const pubkey = secp.ProjectivePoint.fromPrivateKey(
    new Uint8Array(normalizePrivateKeyBytes(priKeyBytes))
  );
  const x = pubkey.x;
  const y = pubkey.y;
...
export function ECKeySign(hashBytes: BytesLike, priKeyBytes: BytesLike) {
  const signature = secp.sign(
    byteArray2hexStr(hashBytes),
    byteArray2hexStr(priKeyBytes)
  );
