...
export function encodeParamsV2ByABI(funABI: ABIType, args: any[]) {
  ...
  const buildFullTypeDefinition = (typeDef: ParamType) => {
...
export function decodeParamsV2ByABI(
  funABI: ABIType,
  ...
  const buildFullTypeNameDefinition = (typeDef: ParamType) => {
  ...
  const decodeResult = (outputs: ReadonlyArray<ParamType>, result) => {
    if (outputs.length)
