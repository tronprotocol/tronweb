...
const regexType = new RegExp(
  "^(trcToken|address|bool|bytes([0-9]*)|string|u?int([0-9]*))$"
);
    ...
    if (type) {
      ...
      if (type === "OPEN_PAREN") {
        ...
        if (tokens.length > 0 && tokens[tokens.length - 1].type === "NUMBER") {
          ...
          suffix = value + suffix;
          (<Writeable<Token>>tokens[tokens.length - 1]).value =
            getNumber(value);
        }
        if (
          tokens.length === 0 ||
...
export class ParamType {
  ...
  static from(obj: any, allowIndexed?: boolean): ParamType {
    ...
    if (
      ...
      const comps =
        obj.components != null
          ? obj.components.map((c: any) => ParamType.from(c))
          : null;
...
export class ConstructorFragment extends Fragment {
  ...
  static from(obj: any): ConstructorFragment {
    ...
    if (typeof obj === "string") {
      ...
      const inputs = consumeParams(obj);
      const payable = !!consumeKeywords(obj, setify(["payable"])).has(
        "payable"
      );
