...
export default class Contract {
    constructor(accWeb = false, abi = [], address = false) {
    ...
    async _getEvents(options = {}) {
        const events = await this.accWeb.event.getEventsByContractAddress(this.address, options);
    ...
    async new(options, privateKey = this.accWeb.defaultPrivateKey, callback = false) {
        ...
        try {
            const address = this.accWeb.address.fromPrivateKey(privateKey);
            const transaction = await this.accWeb.transactionBuilder.createSmartContract(options, address);
            const signedTransaction = await this.accWeb.trx.sign(transaction, privateKey);
            const contract = await this.accWeb.trx.sendRawTransaction(signedTransaction);

            if (contract.code)
                return callback({
                    error: contract.code,
                    message: this.accWeb.toUtf8(contract.message)
                })
    ...
    async at(contractAddress, callback = false) {
        ...
        try {
            const contract = await this.accWeb.trx.getContract(contractAddress);
