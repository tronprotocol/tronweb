...
describe('#contract.readWrite', function () {
    ...
    describe('#namespaces', function () {
        ...
        it('refuses writes when a default address is set but no signing key is available', async function () {
            const tw = new TronWeb({ fullHost: FULL_NODE_API });
            tw.setAddress(accounts.b58[1]); // default address present, but no private key to sign with
            const contract = tw.contract(contractAbi, contractAddress);

            // Must surface the friendly "requires a signer" error rather than a cryptic
            // downstream "Private key does not match address" from trx.sign.
            await assertThrow(
                contract.write.transfer([accounts.b58[0], 1]),
                'Method "transfer" modifies state and requires a signer. Set a private key or default address on the TronWeb instance.'
            );
        ...
        it('rejects a write whose account is not a private key', async function () {
            const contract = tronWeb.contract(contractAbi, contractAddress);

            // `account` must be a private key; a plain address is no longer accepted.
            await assertThrow(
        ...
        it('derives the signer from a private key and signs with that key (write)', async function () {
            ...
            tw.trx.sign = (async (transaction: any, privateKey?: string) => {
                signedWith = privateKey;
                return { ...transaction, signature: ['00'] };
            }) as typeof tw.trx.sign;
            tw.trx.sendRawTransaction = (async (signedTransaction: any) => ({
                result: true,
                transaction: signedTransaction,
            })) as typeof tw.trx.sendRawTransaction;

            // The PK differs from the instance default, so this exercises a non-default
            // signer: the owner address is derived from the PK and the PK signs.
            const txId = await contract.write.transfer([accounts.b58[0], 1], { account: accounts.pks[1] });
            assert.equal(txId, 'cafe');
            assert.equal(captured?.issuerAddress, accounts.b58[1]);
            assert.equal(signedWith, accounts.pks[1]);
        });

        it('allows a write with no default signer when a private key is supplied (write)', async function () {
            const tw = new TronWeb({ fullHost: FULL_NODE_API });
            const contract = tw.contract(contractAbi, contractAddress);
        ...
        it('uses the from option as the caller address (read)', async function () {
            const tw = tronWebBuilder.createInstance();
            const contract = tw.contract(contractAbi, contractAddress);
            ...
            await contract.read.balanceOf([accounts.b58[0]], { from: accounts.b58[1] });
            assert.equal(captured?.issuerAddress, accounts.b58[1]);
        });
    ...
    describe('against a contract deployed to the fullNode', function () {
        ...
        it('signs an on-chain write with a non-default account private key', async function () {
            const contract = tronWeb.contract(fixture.abi, deployedAddress);
            const altStruct: [string, string, string] = [accounts.b58[2], accounts.b58[3], accounts.b58[1]];

            // accounts.pks[1] is not the instance default key: the write must build with
            // accounts.b58[1] as owner and be signed by that key for the node to accept it.
            const txId = await contract.write.setStruct([altStruct], {
            ...
            const tx = (await tronWeb.trx.getTransaction(txId)) as any;
            const owner = tx.raw_data.contract[0].parameter.value.owner_address as string;
            assert.equal(addrBody(owner), addrBody(accounts.b58[1]));
        });
    });
});
