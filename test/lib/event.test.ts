...
describe('TronWeb.lib.event', async function () {
    ...
    before(async function () {
        ...
        contract = tronWeb.contract(abi, contractAddress);
    });
    ...
    describe('#getEventsByContractAddress', async function () {
        ...
        it('should emit an event and wait for it', async function () {
            ...
            assert.isAtLeast(events.data!.length, 1);
        });
    });
    ...
    describe('#getEventsByBlockNumber', async function () {
        ...
        it('should emit an event and wait for it', async function () {
            ...
            let events;
            while (true) {
                events = await tronWeb.event.getEventsByBlockNumber(transactionInfo.blockNumber);
                if (events.data?.length) {
                    break;
                }
                await wait(0.5);
            ...
            const eventData = events.data!.find((event) => 
                event.event_name === 'SomeEvent' && 
                event.contract_address === tronWeb.address.fromHex(contractAddress)
            );
