...
const AccWeb = require('../setup/AccWeb');
...
//     }, sideExtraOptions);
//     return new AccWeb(options, sideOptions);
// }
...
const newTestAccounts = async (amount) => {
    const accWeb = createInstance();

    console.log(chalk.blue(`Generating ${amount} new accounts...`))
    await accWeb.fullNode.request('/admin/temporary-accounts-generation?accounts=' + amount);
...
const getTestAccounts = async (block) => {
    ...
    const accWeb = createInstance();
    const accountsJson = await accWeb.fullNode.request('/admin/accounts-json');
    ...
    for (let i = 0; i < accounts.pks.length; i++) {
        let addr = accWeb.address.fromPrivateKey(accounts.pks[i]);
        accounts.b58.push(addr);
        accounts.hex.push(accWeb.address.toHex(addr));
    }
