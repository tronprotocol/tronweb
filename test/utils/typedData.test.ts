...
import diskUtils from '../testcases/src/disk-utils';
...
describe('TronWeb.utils.typedData', function () {
    describe('#EIP-712', function () {
        ...
        tests.forEach((test: any) => {
            it(`encoding ${test.name}`, function () {
                ...
                assert.equal(TronWeb.utils._TypedDataEncoder.getPrimaryType(test.types), test.primaryType, 'getPrimaryType');
                assert.equal(TronWeb.utils._TypedDataEncoder.hash(test.domain, test.types, test.data), test.digest, 'digest');
            });
        });
    });
    ...
    describe('#EIP-712 with trcToken', function () {
        ...
        it('should be the correct hash struct', function () {
            assert.equal(
                TronWeb.utils._TypedDataEncoder.hashStruct('FromPerson', types, value.from),
                '0x73b79ecc2530586800050c46ee7361ed28c013dfa3d062ed216295cbd5e6a55d'
            );
            assert.equal(
                TronWeb.utils._TypedDataEncoder.hashStruct('ToPerson', types, value.to),
